...
from models.schemas import Paper, CritiqueReport
...
class CriticAgent:
    ...
    async def run_async(self, sub_question: str, papers: List[Paper], synthesis: str) -> CritiqueReport:
        """
        Runs quality critique scoring on a synthesis draft asynchronously.
        """
        logger.info(f"CriticAgent starting review asynchronously for sub-question: '{sub_question}'")
        ...
        # 3. Call LLM asynchronously in JSON mode
        try:
            ...
            response_text = await call_aimlabs_async(

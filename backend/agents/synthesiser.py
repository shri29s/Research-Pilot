...
from agents.vector_store import retrieve_relevant_papers
...
class SynthesiserAgent:
    ...
    async def run_async(self, sub_question: str, feedback: Optional[str] = None) -> str:
        """
        Retrieves relevant papers from the vector store and generates a synthesis asynchronously.
        Allows passing feedback from the Critic agent for iterative revisions.
        """
        logger.info(f"SynthesiserAgent starting synthesis asynchronously for: '{sub_question}'")
        
        # 1. Retrieve top 5 relevant papers from ChromaDB (run in thread since collection query is synchronous)
        papers = await asyncio.to_thread(retrieve_relevant_papers, sub_question, top_k=5)
        ...
        try:
            synthesis = await call_aimlabs_async(
                system_instruction=system_instruction,
                prompt=prompt,
                json_mode=False
            )
            logger.info(f"Successfully generated synthesis asynchronously for: '{sub_question}' (length={len(synthesis)})")
            return synthesis
